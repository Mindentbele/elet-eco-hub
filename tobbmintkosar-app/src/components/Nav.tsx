  ...
  return (
    <header className="fixed top-0 inset-x-0 z-40 bg-cream-50/80 backdrop-blur border-b border-cream-200">
      <nav className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
        <button onClick={() => scrollToId("top")} className="flex items-center gap-2 font-display text-xl font-extrabold text-leaf-700">
          {content.logoDataUrl ? (
        ...
        <ul className="hidden md:flex items-center gap-7 text-sm font-medium text-ink-800">
          {content.navLinks.map(l => (
            <li key={l.id}>
              <button onClick={() => scrollToId(l.target)} className="hover:text-tomato-600">{l.label}</button>
            </li>
          ))}
        </ul>
