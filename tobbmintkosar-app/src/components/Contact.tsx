  ...
  return (
    <section id="contact" className="py-20 md:py-28 bg-gradient-to-br from-leaf-600 to-leaf-700 text-white">
      <div className="max-w-5xl mx-auto px-5 text-center">
        <h2 className="display text-3xl md:text-5xl font-extrabold">{content.contactTitle}</h2>
        <p className="mt-4 text-white/85 text-lg whitespace-pre-line">{content.contactSubtitle}</p>
        <div className="mt-10 grid sm:grid-cols-3 gap-5 max-w-3xl mx-auto">
          ...
          <div className="bg-white/10 backdrop-blur rounded-2xl p-6 border border-white/20">
            ...
            <div className="text-sm text-white/80 mt-1 whitespace-pre-line">{content.contactAddress}</div>
          </div>
        </div>

        {content.mapEmbedUrl && (
          <div className="mt-10 max-w-4xl mx-auto rounded-2xl overflow-hidden shadow-2xl border border-white/20">
            <iframe src={content.mapEmbedUrl} title="Térkép" className="w-full h-[360px]" loading="lazy" referrerPolicy="no-referrer-when-downgrade" />
          </div>
        )}
